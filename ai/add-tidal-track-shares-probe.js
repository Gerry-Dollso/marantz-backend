...
const functionAnchor = `  async function probeTrackProvenance(trackId) {
    if (!/^\\d+$/.test(trackId || '')) {
...
const functionReplacement = functionAnchor + `
  async function probeTrackShares(trackId) {
    if (!/^\\d+$/.test(trackId || '')) {
      throw new Error('Track id must contain digits only');
    }
    ...
    return apiGetRaw(
      '/tracks/' + encodeURIComponent(trackId) +
      '?include=' + encodeURIComponent('shares') +
      '&countryCode=' + encodeURIComponent(countryCode)
    );
  }
`;
...
replaceExactlyOnce(
  'track shares function',
  functionAnchor,
  functionReplacement
);

const routeAnchor = `    if (req.method === 'GET' && requestUrl.pathname === '/api/tidal/oauth/probe-track-provenance') {
      try {
...
const routeReplacement = routeAnchor + `
    if (req.method === 'GET' && requestUrl.pathname === '/api/tidal/oauth/probe-track-shares') {
      try {
        const trackId = requestUrl.searchParams.get('id') || '';
        const track = await probeTrackShares(trackId);
        return sendJson(res, 200, { ok: true, track });
      } catch (error) {
        return sendJson(res, 400, { ok: false, error: error.message });
      }
...
console.log('Added read-only TIDAL track shares probe.');
