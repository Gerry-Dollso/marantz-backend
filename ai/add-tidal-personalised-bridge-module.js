...
// Guarded first productionization step for the official-TIDAL -> HEOS bridge.
// This migration ONLY adds a reusable resolver module. It does not edit
// server.js, does not restart services, and cannot issue playback/queue commands.

const fs = require('fs');
const path = require('path');

const root = path.resolve(__dirname, '..');
const targetPath = path.join(root, 'tidal-heos-resolver.js');
...
if (fs.existsSync(targetPath)) {
  ...
  if (existing === content) {
    console.log('Already applied: tidal-heos-resolver.js matches expected content.');
    process.exit(0);
  }
  throw new Error('Refusing to overwrite existing tidal-heos-resolver.js');
}

fs.writeFileSync(targetPath, content);
console.log('Created tidal-heos-resolver.js');
console.log('No server.js or playback/queue code touched.');
