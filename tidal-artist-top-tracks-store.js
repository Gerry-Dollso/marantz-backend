...
const DEFAULT_ROOT = '/var/lib/marantz-backend/artist-top-tracks';
const DEFAULT_FRESH_MS = 24 * 60 * 60 * 1000;
const DEFAULT_MAX_STALE_MS = 30 * 24 * 60 * 60 * 1000;

function createTidalArtistTopTracksStore(options = {}) {
  const root = path.resolve(options.root || DEFAULT_ROOT);
  const freshMs = Math.max(0, Number(options.freshMs ?? DEFAULT_FRESH_MS));
  const maxStaleMs = Math.max(freshMs, Number(options.maxStaleMs ?? DEFAULT_MAX_STALE_MS));
  ...
  function read(artistId) {
    ...
    try {
      ...
      if (parsed?.version !== 1 || String(parsed.artistId) !== id || !Array.isArray(parsed.value)) return null;
      ...
      if (error.code !== 'ENOENT') console.warn('TIDAL Artist Top Tracks persistent cache read failed:', error.message);
      return null;
    }
  }
  ...
  function write(artistId, value, createdAt = Date.now()) {
    const id = validateId(artistId);
    if (!Array.isArray(value)) throw new Error('Top Tracks cache value must be an array');
  ...
  function stats() {
    ...
    return { root, entries, bytes, freshMs, maxStaleMs };
  }

  return { read, write, remove, stats };
}

module.exports = { createTidalArtistTopTracksStore };
